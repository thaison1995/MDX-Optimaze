...
string ReplaceText(string str, string szFind, string NewStr)
{
...
int handleFile(const char *fname, uint32_t bitmask, uint8_t forceLinear, float threshold) {
  ...
  if (format) 
  {
	...
	string strbk = buffer;
	string strnew = ReplaceText(buffer, "input", "output");
	string strnew2 = ReplaceText(strnew, "_v2.", ".");
	CopyFile(strbk.c_str(), strnew2.c_str(), TRUE);
	DeleteFile(strbk.c_str());
  }
...
int main(int argc, char *argv[]) 
{
 // int32_t i;
  ...
  handleDir("input/", bitmask, forceLinear, threshold);
 ...
 //     } 
	//  else 
	//  {
	//	 handleDir("input\\", 16, 0, threshold);
 //       /*if (isDir(argv[i])) 
	//	{
